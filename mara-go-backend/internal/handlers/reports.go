...
func (h *ReportHandler) Index(w http.ResponseWriter, r *http.Request) {
	...
	if search := r.URL.Query().Get("search"); search != "" {
		like := "%" + search + "%"
		q = q.Where("reference LIKE ? OR description LIKE ? OR region LIKE ?", like, like, like)
	}
...
// POST /reports/{id}/assign — assign report to a coordinator
func (h *ReportHandler) Assign(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	var report models.Report
	if err := h.db.First(&report, id).Error; err != nil {
		jsonError(w, "report not found", http.StatusNotFound)
	...
	if err := decode(r, &req); err != nil || req.AssignedTo == 0 {
		jsonError(w, "assigned_to (user id) required", http.StatusBadRequest)
		return
	}
	updates := map[string]interface{}{
	...
	h.db.Model(&report).Updates(updates)
	h.db.Preload("ViolenceTypes").Preload("Coordinator").First(&report, id)
	h.hub.Broadcast(ws.Event{Type: "report_assigned", Payload: report})
	jsonOK(w, report)
}
